...
namespace Plant.Tests
{
    ...
    public class BasePlantTest
    {
        ...
        [Test]
        [ExpectedException(typeof(PropertyNotFoundException), ExpectedMessage = "Property #Foo with value $bar")]
        public void Should_Throw_PropertyNotFound_Exception_When_Given_Invalid_Property()
        {
            var plant = new BasePlant();
            plant.DefinePropertiesOf<Person>(new { Foo = "bar" });
            plant.Create<Person>();
        }

        [Test]
        [ExpectedException(typeof(TypeNotSetupException), ExpectedMessage = "No creation strategy defined for type: Plant.Tests.TestModels.Person")]
